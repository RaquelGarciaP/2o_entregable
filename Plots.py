...
for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],metatable[ii]['col4'],
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color
...
plt.ylabel(r'$\log(\tau_{\rm Ross})$', fontsize='12')
plt.xlabel(r'$r$', fontsize='12')   

plt.gca().invert_xaxis()
plt.show()
#%%
Pe_Pg = [metatable[0]['col6']/metatable[0]['col7'],metatable[1]['col6']/metatable[1]['col7']]
Prad_Pg = [metatable[0]['col8']/metatable[0]['col7'],metatable[1]['col8']/metatable[1]['col7']]

for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],metatable[ii]['col5'],
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color
    
plt.legend(fontsize=12)   
plt.ylabel(r'T[K]', fontsize='12')
plt.xlabel(r'$\log(\tau_{\rm Ross})$', fontsize='12') 
plt.show()
#%%
for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],np.log10(metatable[ii]['col6']),
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color

plt.legend(fontsize=12)   
plt.ylabel(r'$\log(P_{e}[Ba])$', fontsize='12')
plt.xlabel(r'$\log(\tau_{Ross})$', fontsize='12') 
plt.show()
...
#%%    
for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],Pe_Pg[ii],
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color
...
#%%
for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],Prad_Pg[ii],
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color
...
#%%
tau_ross_t5000 = 10**metatable[0]['col2']
tau_ross_t8000 = 10**metatable[1]['col2']

T = [((3/4)*(5000**4)*(10**metatable[0]['col2'] + (2/3)))**(1/4),((3/4)*(8000**4)*(10**metatable[1]['col2'] + (2/3)))**(1/4)]

for ii in range (0,len(metatable)):
    plt.plot(metatable[ii]['col2'],T[ii],
    label=f"{names[ii]}",  # Etiqueta para la leyenda
    color=colors[ii])  # Asignar un color
