...
aa = [1.99654, -1.18267e-5, 2.64243e-6, -4.40524e-10, 3.23992e-14, -1.39568e-18, 2.78701e-23]

# function to calculate the opacities
def calculate_opacities(lgTauR, lgTau5, depth, T, Pe, Ne, N_ionization, N_excitation):
    ...
    for n, N_HI_nn in zip(nn, N_excitation):
        ...
        g_bf = 1 + (0.3456 / (lamb[lamb_condition] * R)**3) * ((lamb[lamb_condition] * R)/ n**2 - 1/2)
        sigma_bf_HI = 2.815e29 * g_bf / (n**5 * nu[lamb_condition]**3)
        
        # Update k_bf_HI only where condition is met
        k_bf_HI[lamb_condition] = sigma_bf_HI * N_HI_nn * (1- np.exp(-h*nu[lamb_condition]/(kb*T)))
    ...
    def line_absorption_coef(u, l, nu, nl,  delta):
        g_bb = np.pi * np.sqrt(3) * abs((((u-l)/(u+l))**(2*u+2*l) * u*l * delta) / (u-l))
        f = 2**5 * g_bb / (3**(3/2) * np.pi * l**5 * u**3) * (1/l**2 - 1/u**2)**(-3)
        sigma_bb = np.pi * e**2 * f / (m * c)

        gu = 2*u**2
        gl = 2*l**2

        k_bb = sigma_bb * (nl - gl/gu * nu)
    ...
    # free-free
    f0 = -2.2763 - 1.6850*np.log10(lamb_A) + 0.76661*((np.log10(lamb_A))**2) - 0.053346*((np.log10(lamb_A))**3)
    f1 = 15.2827 - 9.2846*np.log10(lamb_A) + 1.99381*((np.log10(lamb_A))**2) - 0.142631*((np.log10(lamb_A))**3)
    f2 = -197.789 + 190.266*np.log10(lamb_A) - 67.9775*((np.log10(lamb_A))**2) + 10.6913*((np.log10(lamb_A))**3) - 0.625151*((np.log10(lamb_A))**4)

    sigma_ff_Hminus = 1e-26 * 10**(f0+f1*np.log10(theta)*f2*(np.log10(theta))**2)
    ...
    # Condition for lamb values
    lamb_condition = lamb <= (1 / R)  # Boolean mask for lamb condition

    # Initialize k_bf_Hminus to zeros
    k_bf_Hminus = np.zeros_like(lamb)

    sigma_bf_Hminus = (aa[0]+aa[1]*lamb_A[lamb_condition]+aa[2]*lamb_A[lamb_condition]**2+aa[3]*lamb_A[lamb_condition]**3+aa[4]*lamb_A[lamb_condition]**4+aa[5]*lamb_A[lamb_condition]**5+aa[6]*lamb_A[lamb_condition]**6) * 1e-18

    # Update k_bf_Hminus only where condition is met
    k_bf_Hminus = 4.158e-10 * sigma_bf_Hminus * Pe * theta**(5/2) * 10**(0.754*theta)
    ...
    k_TOTAL = k_ff_HI + k_bf_HI_n1 + k_bf_HI_n2 + k_bf_HI_n3 + k_ff_Hminus + k_bf_Hminus + k_es
